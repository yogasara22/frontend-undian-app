...
export function DrawButton({ isRolling, isLoading, hasWinner, onDraw, onReset }: DrawButtonProps) {
  ...
  if (hasWinner) {
    return (
      <motion.button
        ...
        whileTap={{ scale: 0.97 }}
        className="relative px-8 py-4 rounded-2xl font-bold text-sm text-gray-700 bg-white shadow-sm hover:shadow-md transition-all border border-gray-200 flex items-center gap-2 group"
      >
  ...
  return (
    <motion.button
      ...
      className="relative px-8 py-4 rounded-2xl font-bold text-base text-white shadow-xl hover:shadow-2xl transition-all overflow-hidden flex items-center gap-3 cursor-pointer"
      style={{
      ...
      <div className="relative z-10 flex items-center gap-3">
        {isLoading ? (
          ...
          <>
            ...
            <span className="tracking-wide font-extrabold pr-2">Mulai Undian</span>
          </>
        )}
      </div>
    </motion.button>
